...
class pm_predictor : public branch_predictor {
	...
	unsigned ip_msk_a_hash;
	unsigned ip_msk_b_hash;
	unsigned ip_msk_c_hash;
	unsigned pir_msk_a_hash;
	unsigned pir_msk_b_hash;
	unsigned taken_mask_hash;
	...
	pm_predictor (void) { // CONSTRUCTOR
		...
		for (int i = 0; i < GLOBAL_WAYS; i++) { // INITIALIZE ALL GLOBAL PREDICTOR LAST USED TO 0
			for (int j = 0; j < GLOBAL_ROWS; j++) {
				global_predictor[i][j].prevused = 0;
		...
		// Create Hash masks
		ip_msk_a_hash = createMask(13,18);
		ip_msk_b_hash = createMask(4,12);
		ip_msk_c_hash  = createMask(10,18);
		pir_msk_a_hash = createMask(0,5);
		pir_msk_b_hash = createMask(6,14);
		taken_mask_hash = createMask(0,5);
	} // END OF CONSTRUCTOR
	...
	branch_update *predict (branch_info & b) {
		...
		if (b.br_flags & BR_CONDITIONAL) {
			unsigned  hash = ((((b.address & ip_msk_a_hash)>>13) ^ (history & pir_msk_a_hash) )<<9) + (((b.address & ip_msk_b_hash)>>4)^((history & pir_msk_b_hash)>>6));
	...
	void update (branch_update *u, bool taken, unsigned int target) {
		...
		history = ((history & pir_update_mask) << 2 ) ^ ((BR_CONDITIONAL*(bi.address & ip_pir_update_mask)) | (BR_INDIRECT*((bi.address & ip_msk_c_hash >> 5) + (target & taken_mask_hash))));
			...
			if (((pm_update*)u)->usedGlobal) {
				for (int i = 0; i < GLOBAL_WAYS; i++) {
					if (global_predictor[i][index].tag == tag) {
						...
						if (global_predictor[i][index].prevused < 5) {
							global_predictor[i][index].prevused++;
						}
						break;
					}
				}
				...
				bool swap = false;
				for (int i = 0; i < GLOBAL_WAYS; i++) {
					if (global_predictor[i][index].prevused > 0) {
						global_predictor[i][index].prevused--; // all misses, reduce them all
					}
					else if (global_predictor[i][index].prevused <= 0 && !swap) {  // prevused is zero, use it
						...
						global_predictor[i][index].prevused = 5; // reset the most used policy to max
						swap=true;
					}
				}
			}
